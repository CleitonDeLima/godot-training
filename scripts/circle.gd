...
# ataque
@export var attack_interval: float = 0.0   # tempo entre ataques
...
func _process(delta: float) -> void:
	...
	if not attacking and attack_timer:
		attacking = true
		attack_timer = 0.0
		attack_progress = 0.0
...
func _draw() -> void:
	...
	if attacking:
		var current_radius: Variant = lerp(0.0, attack_max_radius, attack_progress)
